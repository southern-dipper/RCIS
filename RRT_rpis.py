...
def signed_angle_difference(theta2, theta1):
    """计算从theta1到theta2的有符号最短角度差（结果在[-pi, pi]）"""
...
def rrt_search(start_indices: Tuple[int, int, int], goal_xy_indices: Tuple[int, int], 
               ...
               max_iterations: int = 3000, goal_tolerance: float = 1.0, 
    ...
    for iteration in range(max_iterations):
        ...
        # *** 新增: 检查角速度约束 ***
        required_omega = signed_angle_difference(new_state[2], nearest_node.state[2]) / DT
        if not (OMEGA_MIN <= required_omega <= OMEGA_MAX):
            continue # 角速度超出范围，放弃
...
def safe_rrt_search(start_indices, goal_xy_indices, S_infinity, obstacle_indices, max_iterations=3000, goal_tolerance=1.0, goal_bias_prob=0.1):
    ...
    for iteration in range(max_iterations):
        ...
        # 5. 状态扩展 (已修正)
        max_step = V * DT
        # 扩展方向由修正后的安全角度theta_target决定
        direction = np.array([np.cos(theta_target), np.sin(theta_target)])
        # 扩展距离参考原始采样点，但不超过最大步长
        dist_to_extend = min(min_dist, max_step)
        q_new = q_near_pos + dist_to_extend * direction
        
        # 验证扩展后状态的安全性
        new_state = np.array([q_new[0], q_new[1], theta_target])
        
        # *** 新增: 检查角速度约束 ***
        required_omega = signed_angle_difference(theta_target, nearest_node.state[2]) / DT
        if not (OMEGA_MIN <= required_omega <= OMEGA_MAX):
            continue # 角速度超出范围，放弃
        ...
        if (ix_new, iy_new, itheta_new) in S_infinity:
            ...
            if best_safe_state is not None:
                # --- 修正后的局部搜索逻辑 ---
                # 1. 计算从父节点到候选位置的实际“来向角度”
                theta_approach = np.arctan2(best_safe_state[1] - nearest_node.state[1], 
                                            best_safe_state[0] - nearest_node.state[0])

                # 2. 用“来向角度”构建最终要存入节点的状态
                final_node_state = np.array([best_safe_state[0], best_safe_state[1], theta_approach])

                # 3. 对这个最终状态检查运动学约束
                required_omega = signed_angle_difference(theta_approach , nearest_node.state[2]) 
                if not (OMEGA_MIN <= required_omega <= OMEGA_MAX):
                    correction_stats['failed'] += 1
                    continue # 角速度超出范围

                # 检查距离是否合理
                distance_2d = np.sqrt((final_node_state[0] - nearest_node.state[0])**2 + 
                                    (final_node_state[1] - nearest_node.state[1])**2)
                if distance_2d < eta * max_step:
                    path_collision = check_path_collision(nearest_node.state, final_node_state, obstacle_indices)
                    if path_collision:
                        continue
                    new_node = RRTNode(final_node_state, nearest_node)
                    nearest_node.add_child(new_node)
                    tree_nodes.append(new_node)
...
def create_rrt_path_visualization(S_infinity, obstacle_indices, rrt_result, safe_rrt_result, start_continuous, goal_continuous, safe_angle_count):
    """RRT path planning visualization - supports baseline and safe RRT comparison"""
    ...
    if safe_rrt_result and safe_rrt_result[0] is not None:
        ...
        # Draw actual continuous trajectory
    ...
    # Draw goal region (circular area, radius 1.0, consistent with goal detection logic)
    ...
    legend_patches = [
        ...
        plt.Line2D([0], [0], color='#FFB3B3', linewidth=1.2, alpha=0.8, label='Base Tree'),
        plt.Line2D([0], [0], color='#B3D9FF', linewidth=1.2, alpha=0.8, label='Safe Tree'),
    ...
    ax.legend(handles=handles + legend_patches, fontsize=9, markerscale=0.7,
...
算法2: 基线RRT路径规划（带角速度约束）
输入: 起点索引, 目标XY索引, 安全集, 障碍物索引集合, 最大迭代数, 目标容忍度, 目标偏向概率
输出: 路径, 树节点列表, 搜索时间
...
2. 主循环（最大迭代数次）：
   - 采样策略：
     * 概率性目标偏向：以goal_bias_prob概率直接采样目标点
     * 否则在二维空间[X_MIN,X_MAX]×[Y_MIN,Y_MAX]内均匀随机采样
   - 最近邻搜索：遍历树中所有节点，找到与采样点二维欧氏距离最小的节点
   - 扩展方向计算：计算从最近节点指向采样点的角度theta_toward_sample
   - 运动学约束检查：
     * 计算所需角速度: required_omega = (theta_new - theta_old) / DT
     * 如果超出角速度范围[OMEGA_MIN, OMEGA_MAX]，放弃当前采样
   - 状态扩展：
     * 计算二维扩展距离，限制在最大步长max_step = V*DT内
     * 生成新的三维状态[x_new, y_new, theta_toward_sample]
   - 有效性检查：
     * 边界检查：确保新状态在有效区域内
     * 路径碰撞检查：检查从最近节点到新状态的路径
   - 树扩展：创建新节点，建立父子关系，加入树
   - 目标检查：使用圆形区域检测(欧氏距离≤goal_tolerance)是否到达目标

算法3: 安全RRT路径规划（基于RCI约束的两阶段扩展策略）
输入: 起点索引, 目标位置, 鲁棒控制不变集S_∞, 障碍物格元集合, 最大迭代数, 目标容忍度, 目标偏向概率
输出: 连续状态路径, 树节点列表, 搜索时间

1. 初始化与安全检查：
   - 将起点索引转换为连续状态空间中的状态
   - 验证起点是否属于RCI集S_∞（通过格元复形映射）
   - 如果起点不在安全集内，返回路径规划失败
   - 创建根节点，初始化搜索树和局部修正统计信息

2. 主循环迭代（直至达到最大迭代数或找到路径）：
   a) 目标偏向采样策略：
      * 以概率goal_bias_prob采样目标点坐标
      * 否则在连续配置空间X̃ = [X_MIN,X_MAX] × [Y_MIN,Y_MAX]内均匀随机采样
   
   b) 最近邻查询：
      * 计算采样点q_rand与树中所有节点的二维欧氏距离
      * 选择距离最小的节点q_near作为扩展基点
   
   c) 期望运动方向计算：
      * 计算期望朝向角θ_desired = arctan2(q_rand.y - q_near.y, q_rand.x - q_near.x)
      * 确保角度计算的数值稳定性和象限正确性
   
   d) 运动学约束预验证：
      * 计算所需角速度ω = (θ_desired - θ_near) / Δt
      * 验证ω ∈ [ω_min, ω_max]，不满足则放弃当前采样点
   
   e) 状态扩展计算：
      * 计算单位方向向量û = [cos(θ_desired), sin(θ_desired)]
      * 确定扩展步长δ = min(‖q_rand - q_near‖, v_max · Δt)
      * 生成候选新状态q_new = [q_near.x + δû.x, q_near.y + δû.y, θ_desired]
   
   f) 两阶段安全验证机制：
      【阶段1 - 直接RCI验证】
      * 将q_new映射到格元复形S中的离散索引(i_x, i_y, i_θ)
      * 检查(i_x, i_y, i_θ) ∈ S_∞（RCI成员资格测试）
      * 若安全，执行路径碰撞检测，通过则加入树并继续
      
      【阶段2 - 局部修正搜索】（当阶段1失败时启动）
      * 在q_new对应格元的3×3×3邻域内搜索（27个候选格元）
      * 对每个邻域格元(i_x+Δi_x, i_y+Δi_y, i_θ+Δi_θ)：
        - 验证其RCI成员资格：是否属于S_∞
        - 计算归一化三维距离度量作为质量指标
      * 选择最近的安全格元作为修正目标
      * 重新计算运动一致的朝向角θ_approach基于实际运动方向
      * 验证修正后状态的运动学可达性（角速度约束）
      * 验证修正后的步长合理性（距离约束η·v_max·Δt）
      * 执行路径碰撞检测，通过则加入修正节点到树
   
   g) 目标区域检测：
      * 计算当前节点到目标位置的二维欧氏距离
      * 若距离 ≤ goal_tolerance，触发路径重构
...
算法特点与理论保证：
1. 鲁棒安全性：所有路径点保证属于预计算的RCI集S_∞，确保在扰动下的安全性
2. 两阶段扩展：结合直接扩展的效率与局部修正的鲁棒性
3. 运动学一致性：严格满足系统的速度和角速度约束
4. 格元复形映射：连续状态与离散安全集的精确对应关系
5. 形式化保证：基于不变集理论的安全性证明
...
#         W: 扰动集
#         goal_continuous: 目标位置连续坐标 [x, y]
#         goal_tolerance: 目标区域半径（默认1.0）
    
#     Returns:
#         set: 鲁棒安全集
#     """
#     print("计算鲁棒安全集（目标区域黑洞模式）...")
...
#                 if is_state_valid(x_actual, y_actual):
#                     for itheta in range(len(theta_space)):
#                         state_indices = (ix, iy, itheta)
#                         S_safe.add(state_indices)
                        
#                         # 检查是否在目标区域内
#                         if abs(ix-goal_xy_indices[0]) <= goal_tolerance and abs(iy-goal_xy_indices[1]) <= goal_tolerance:
#                             goal_states.add((ix, iy,itheta))
    
#     print(f"初始安全集大小: {len(S_safe)}")
#     print(f"目标区域状态数: {len(goal_states)}")
...
#             # *** 黑洞处理：如果当前状态在目标区域内，直接加入下一轮安全集 ***
#             if s_indices in goal_states:
#                 Sk_plus_1.add(s_indices)
#                 continue
...
#                     # 安全集检查
#                     if s_next_indices in goal_states:
#                         break
#                     if s_next_indices not in Sk:
#                         is_action_robustly_safe = False
#                         break
...
#         if Sk_plus_1 == Sk:
#             print(f"鲁棒安全集已收敛: {len(Sk_plus_1)} 个状态")
#             print(f"其中目标区域状态: {len(goal_states & Sk_plus_1)} 个")
#             return Sk_plus_1
