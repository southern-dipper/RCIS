...
In this paper, we introduced a novel framework that enhances classical path-planning algorithms with provable safety guarantees by leveraging the power of Robust Control Invariant Sets (RCIS). We addressed the critical limitation of popular planners like A* and RRT, whose performance degrades under model uncertainty and external disturbances. Our core contribution is a practical methodology that decouples the computationally intensive safety analysis from the online planning phase. By pre-computing the maximal RCIS and a corresponding Safe Action Set, we provide planners with an efficient, constant-time mechanism to verify and enforce robust safety during execution.
...
Future work will proceed along several promising directions. First, while effective, the grid-based RCIS computation faces scalability challenges with higher-dimensional systems. We plan to explore more scalable techniques, such as symbolic methods or function approximation, to extend our framework to more complex dynamics. Second, the current approach assumes a static environment; future research will focus on developing methods for efficiently updating the RCIS or adapting the safe action set in response to dynamic obstacles. Finally, we aim to validate the proposed framework on physical hardware to demonstrate its real-world applicability and robustness in safety-critical robotic applications.
